...
Drop these files into `public/`, then rebuild `components/layout/Logo.tsx` to use `<Image>` instead of the text-mark (see CLAUDE.md for the full `variant`-prop pattern):

- [ ] `logo.svg` — primary logo, SVG preferred (scales perfectly)
- [ ] `logo.png` — PNG fallback (min 400×120px, transparent background)
- [ ] `logo-white.svg` — white/reversed version for dark backgrounds (footer)
- [ ] `logo-white.png` — white PNG fallback
- [ ] `favicon.ico` — classic favicon (32×32px, multi-size .ico; replace the current empty placeholder)
- [ ] `favicon-16x16.png`
- [ ] `favicon-32x32.png`
...
`site.webmanifest` is already wired up at `public/site.webmanifest` — update `theme_color` / `background_color` if the final brand palette changes.

### Spec sheets (`public/specs/`)
...
After deploying to https://bluegou.com:

1. Go to https://search.google.com/search-console
2. Click **Add Property → URL prefix** → enter `https://bluegou.com`
3. Choose **HTML tag** verification method
4. Copy the verification code (looks like `abc123xyz`)
5. In [app/layout.tsx](./app/layout.tsx), replace `PASTE_GOOGLE_VERIFICATION_CODE_HERE` with the real code
6. Also rename `public/googlePASTE_GOOGLE_VERIFICATION_CODE_HERE.html` to `public/google[YOUR_CODE].html`
7. Redeploy to Vercel
8. Click **Verify** in Search Console
9. Go to **Sitemaps** → add `https://bluegou.com/sitemap.xml`
10. Click **Request Indexing** on the home page first
