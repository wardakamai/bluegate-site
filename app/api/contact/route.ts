...
function buildEmailBody(d: ContactFormData): string {
  return [
    `Name:              ${d.name}`,
    `Company:           ${d.company}`,
    d.companyWebsite ? `Company Website:   ${d.companyWebsite}` : null,
    `Email:             ${d.email}`,
    `Phone:             ${d.phone}`,
    `Country:           ${d.country}`,
    `Product:           ${d.product}`,
    d.productSpec ? `Product Spec:      ${d.productSpec}` : null,
    d.quantity ? `Quantity:          ${d.quantity.toLocaleString()} ${d.unit}` : null,
    `Preferred Port:    ${d.preferredPort}`,
    d.startDate ? `Requested Start:   ${d.startDate}` : null,
    d.periodMonths ? `Storage Duration: ${d.periodMonths} months` : null,
    `Vessel Status:     ${d.vesselStatus}`,
    d.vesselEta ? `Vessel ETA:        ${d.vesselEta}` : null,
    d.transferRequirements ? `Transfer Needs:    ${d.transferRequirements}` : null,
    `Intent:            ${d.intent}`,
